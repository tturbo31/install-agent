...
// A decline that points at the photo itself ("That isn't something we install",
// "This finish is not something we do") instead of naming the finish. Measured
// 2026-09-22 after the website-link prompt change: 4/4 replies took this form,
// while the previous prompt named the finish 4/4 ("That's travertine, not
// something we install"). Both comply with the owner rule (decline, list what
// we do, no visit), so the [2c] check accepts either.
const DEMONSTRATIVE_DECLINE = (t: string) => /\b(?:that|this)(?:\s+\w+){0,2}\s*(?:isn'?t|is\s+not|'s\s+not)\s+something\s+we\s+(?:install|do|offer|work\s+with)\b/i.test(t);
...
async function main() {
  ...
  ck("says that finish isn't something we install", DECLINES_UNSUPPORTED(r3) || DEMONSTRATIVE_DECLINE(r3) || /\b(?:pavers?|stone|travertin[eo]|concrete|cement)\b[^.!?]{0,60}\b(?:not|isn'?t|don'?t)\b|\b(?:not|isn'?t|don'?t)\b[^.!?]{0,60}\b(?:pavers?|stone|travertin[eo]|concrete|cement)\b/i.test(r3), r3);
