...
async function processBookingCommand(
  ...
  try {
    ...
    // book the visit; the name is never asked. A name the client typed is used;
    // otherwise it is looked up on the platform (last visit by phone) / the
    // saved profile below.
    ...
    // Always book under the real client name: a name the client typed, then
    // the name on their last visit in the platform (by phone), then the saved
    // Instagram profile name, then the @handle, then whatever the model wrote.
    // Never just "Client".
    ...
    const clientName = resolveClientName(
      [typedName, platformName, convData?.name, convData?.username, bookingData.name],
      "Instagram Client"
    );
    if (!typedName) console.log(`[IG] booking without a typed name — using ${platformName ? "the platform's" : convData?.name ? "the profile's" : "a fallback"} (${clientName})`);
    ...
    if (result.success) {
      ...
      if (urgent) {
        waitUntil(notifyOwners({ platform: "Instagram", clientName: bookingData.name ?? null, clientId: senderIgsid, recentMessages: history.slice(-6), alert: urgent }).catch((e) => console.error("same-day alert error:", e)));
      }
      ...
      const realVisit = await getUpcomingBookingRecord(senderIgsid).catch(() => null);
...
async function handleWebhook(body: WebhookPayload, opts?: { replay?: boolean }) {
  try {
    ...
    if (!booked && !isBookingConfirmed && !isRescheduling && !/\[BOOK:/i.test(safeAiText)) {
      const retryReason = forcedBookRetryReason(afterBookingText, history, false);
      if (retryReason) {
        console.warn("[IG] " + (retryReason === "claim" ? "visit claimed as scheduled without [BOOK]" : "re-asking booking data the client already gave") + " — forcing a [BOOK] retry");
        const retryText = await retryForBookTag(messagesForAI, memoryContext, systemMemory, ownerCorrections, retryReason);
        if (retryText) {
          const retryStep = await processBookingCommand(retryText, conversation.id, senderIgsid, isBookingConfirmed, lang, isRescheduling, history);
          afterBookingText = retryStep.response;
          booked = retryStep.booked;
        } else if (retryReason === "claim") {
          afterBookingText = bookingUnverifiedHandoffMessage(lang) + "[NOTIFY_OWNER]";
        } else {
          afterBookingText = afterBookingText + "[NOTIFY_OWNER]";
        }
      }
    }
