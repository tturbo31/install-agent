...
async function processBookingCommand(
  ...
  if (mobileHomeStanding(history)) {
    console.warn("[FB] booking blocked — trailer / mobile home (we do not work in them); sending the decline");
  ...
  try {
    ...
    // re-types digits and transposes them (Shaeleen replay, 2026-08-27).
    {
      ...
      if (rp.corrected) {
        console.warn("[FB] booking phone corrected: " + rp.reason);
    ...
    if (isReschedule) {
      ...
      if (r.success && r.unchanged) {
        ...
        console.log("[FB] [BOOK] repeats the existing visit — restating it (nothing to move)");
    ...
    if (result.success) {
      ...
      if (urgent) {
        waitUntil(notifyOwners({ platform: "Facebook Messenger", clientName: bookingData.name ?? null, clientId: `fb_${psid}`, recentMessages: history.slice(-6), alert: urgent }).catch((e) => console.error("same-day alert error:", e)));
      }
      ...
      // com OUTRA data (Raul Gallon, WA 28/08/2026). Restata a visita REAL.
      const realVisit = await getUpcomingBookingRecord(`fb_${psid}`).catch(() => null);
    ...
    console.error("FB booking error — [BOOK] unusable, handing off to owner:", err);
    return { response: bookingFailureHandoffMessage(lang) + "[NOTIFY_OWNER]", booked: false };
  }
}
...
async function handleFbMessage(body: Record<string, unknown>, opts?: { replay?: boolean }) {
  try {
    ...
    if (!thisMessageId && opts?.replay && msgId) {
      ...
      console.log(`[FB] replay of stored message ${thisMessageId || "(not found)"} — lost-reply recovery`);
    ...
    if (isBooked && bookedVisit) {
      ...
      if (isConditionalEarlierRequest(condText) && !isRescheduleRequest(stripConditionalEarlier(condText))) {
        ...
        if (!(lastBotForCond?.content && isConsecutiveDuplicate([{ role: "assistant", content: lastBotForCond.content }], ack))) {
          const ackSent = await sendFacebookMessage(psid, ack);
          if (ackSent.ok) {
            await supabaseAdmin.from("instagram_messages").insert({ conversation_id: conv.id, role: "assistant", content: ack });
          }
        }
        waitUntil(appendUpcomingBookingNote(fbIgsid, `Cliente pediu aviso se abrir horário mais cedo: "${condText.replace(/\s+/g, " ").slice(0, 100)}"`).catch(() => false));
        try {
          const { data: recentMsgs } = await supabaseAdmin
            .from("instagram_messages")
            .select("role, content")
          ...
          console.error("FB earlier-slot ack notify error:", err);
        }
        return;
      }
    }
    ...
    if (isBooked && !engageReschedule) {
      ...
      const corr = await applyPostBookingAddressCorrection(fbIgsid, addrBurst);
      if (corr) {
        ...
        if (!(lastBotAddr?.content && isConsecutiveDuplicate([{ role: "assistant", content: lastBotAddr.content }], reply))) {
          const addrSent = await sendFacebookMessage(psid, reply);
          if (addrSent.ok) {
            await supabaseAdmin.from("instagram_messages").insert({
              conversation_id: conv.id,
              role: "assistant",
        ...
        try {
          ...
          console.error("FB address-correction notify error:", err);
        }
        return;
      }
    }
    ...
    if (isBooked && !engageReschedule && (isReminderRequest(rawText) || isReminderRequest(gateBurst))) {
      ...
      waitUntil(appendUpcomingBookingNote(fbIgsid, `Cliente pediu aviso antes da visita: "${(gateBurst || rawText).replace(/\s+/g, " ").slice(0, 100)}"`).catch(() => false));
      try {
        const { data: recentMsgs } = await supabaseAdmin
          .from("instagram_messages")
          .select("role, content")
        ...
        console.error("FB reminder-ack notify error:", err);
      }
      return;
    }
    ...
    if (!isBookingConfirmed && mobileHomeLeak(history, safeResponse)) {
      console.warn("[FB] trailer / mobile home — model priced / offered a visit / asked details; replacing with the decline");
    ...
    if (!booked && !isBookingConfirmed && !isRescheduling) {
      ...
      if (softened !== afterBooking) console.warn("[FB] visit claimed without a booking behind it — softened to penciling in");
      afterBooking = rewriteBookingDataAsk(softened, history, false, lang);
    }
    ...
    try {
      mainSent = await sendFacebookMessage(psid, outboundResponse);
    } catch (sendErr) {
      ...
      console.error("[FB] final send THREW — treating as failed:", sendErr);
