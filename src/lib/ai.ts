...
// 2026-09-21: the ads gained a second discount button, "Do you offer any
// discounts or promotions?" (19 taps in 15 conversations in 14 days). It was not
// a known FAQ, so faqButtonPlusTypedText read it as TYPED text and sent the
// whole burst to the model: a 355 character reply with three rates, "Good
// questions." as an opener, or the discount question left unanswered.
const AD_FAQ_DISCOUNT = /\bdiscounts?\b[^.!?\n]{0,40}\b(?:large|larger|big|bigger)\s+(?:spaces?|areas?|projects?|jobs?)\b|\b(?:large|larger|big|bigger)\s+(?:spaces?|areas?|projects?)\b[^.!?\n]{0,20}\bdiscounts?\b|\bdescuentos?\b[^.!?\n]{0,40}\b(?:espacios?|[aá]reas?|proyectos?)\s+(?:m[aá]s\s+)?grandes?\b|\bdiscounts?\s+(?:or|and|&)\s+promo(?:tion)?s?\b|\bpromo(?:tion)?s?\s+(?:or|and|&)\s+discounts?\b|\bdescuentos?\s+[oy]\s+promoci[oó]n(?:es)?\b|\bpromoci[oó]n(?:es)?\s+[oy]\s+descuentos?\b/i;
...
The photo analysis shows a concrete, cement, epoxy, microcement, paver, flagstone or terrazzo floor, and the client has NOT named a floor we install. We do NOT do those floors. Before ANY price, slot, visit or booking-details ask, your FIRST sentence must state explicitly, in the client's language, that that kind of finish is not something we install (say it in those words, for example "that isn't something we install"; never skip it and never replace it with a question like "are you looking to put a new floor over it?"), then name what we DO install (luxury vinyl plank with wood or stone look, porcelain and ceramic tile, hardwood, carpet), and ask whether they want one of those (over or instead of that floor). Two short sentences. NEVER offer visit slots, NEVER ask for name, address or phone, NEVER generate [BOOK:...] in this turn. Never say "for a space that size" or infer the project size from the photo.`;
...
// "What is your name please?" while a visit is being set up (2026-09-21): the
// reply came back as "You're texting with the OzziFloors team!" and nothing
// else in 5 runs of 6, the offered times left hanging. Same cure as the number.
const IDENTITY_LINE = /texting\s+with\s+the\s+ozzi\s?floors\s+team|(?:hablando|chateando|escribiendo)\s+con\s+el\s+equipo\s+de\s+ozzi\s?floors|(?:falando|conversando)\s+com\s+a\s+equipe\s+(?:da\s+)?ozzi\s?floors/i;
export function isIdentityOnlyReply(text: string): boolean {
  const t = (text || "").replace(/\[[^\]]*\]/g, " ").replace(/\s+/g, " ").trim();
  return !!t && t.length <= 140 && !t.includes("?") && IDENTITY_LINE.test(t);
}
export function keepBookingThreadAfterPhone(history: Array<{ role: string; content: string }>, text: string, lang: "en" | "es" | "pt"): string {
  if (!isPhoneOnlyReply(text) && !isIdentityOnlyReply(text)) return text;
  const h = history ?? [];
  let prev = "";
  ...
  let tail: string;
  if (details && slots) {
    tail = lang === "pt" ? "E pra deixar a visita marcada, me confirma o horário que funciona melhor e me passa o endereço completo com o zip code e o melhor telefone?"
      : lang === "es" ? "Y para dejar la visita lista, me confirmas el horario que te queda mejor y me pasas la dirección completa con el código postal y tu teléfono?"
      : "And to get the visit set, which of those times works best, and can I get the full address with the zip code and the best phone number?";
  } else if (details) {
    tail = lang === "pt" ? "E pra deixar a visita marcada, me passa o endereço completo com o zip code e o melhor telefone?"
      : lang === "es" ? "Y para dejar la visita lista, me pasas la dirección completa con el código postal y tu teléfono?"
      : "And to get the visit set, can I get the full address with the zip code and the best phone number?";
  } else {
...
// ─── Never CLAIM to be a human (2026-09-21) ─────────────────────────────────
// The owner's tone rule says the client is told they are texting with the
// OzziFloors team, and that is all it says. Asked "are you a real person or a
// bot?", the model went further on its own in 4 replies of 6: "so a real person
// is behind this", "we're real people", "you'll always get a real person on
// this side". That is a false statement to someone asking sincerely, and a
// legal exposure for the company (bot disclosure rules). The reply keeps the
// team line and loses the claim. It never says it is a bot either: nothing is
// added, a sentence is only removed.
const HUMAN_CLAIM = /\b(?:real|actual|live)\s+(?:person|people|humans?)\b|\b(?:i'?m|i\s+am|we'?re|we\s+are)\s+(?:a\s+|all\s+)?humans?\b|\bnot\s+(?:a\s+|an\s+)?(?:bots?|robots?|machine|ai)\b|\bno\s+(?:bots?|robots?)\s+here\b|\bpersonas?\s+(?:real|reales|de\s+verdad)(?![a-zà-ÿ])|\bno\s+(?:soy|somos)\s+(?:un\s+|una\s+)?(?:bots?|robots?|m[aá]quina)(?![a-zà-ÿ])|\bpessoas?\s+(?:real|reais|de\s+verdade)(?![a-zà-ÿ])|\bn[ãa]o\s+(?:sou|somos)\s+(?:um\s+|uma\s+)?(?:bots?|rob[ôo]s?|m[aá]quina)(?![a-zà-ÿ])/i;
// The turn where it applies: the client asked whether they are talking to a
// person or a machine. Anywhere else "a real person comes out to measure" is
// an ordinary sentence and is left alone.
const BOT_OR_HUMAN_QUESTION = /\b(?:bots?|robots?|chat\s?bot|automated|a\.?i\.?|machine|real\s+(?:person|people|human)|(?:a|an\s+actual)\s+human|human\s+being)\b|\bpersona\s+(?:real|de\s+verdad)\b|\bhumano\b|\bm[aá]quina\b|\brob[ôo]\b|\bpessoa\s+(?:real|de\s+verdade)\b/i;
export function clientAskedBotOrHuman(history: Array<{ role: string; content: string }>): boolean {
  for (let i = (history?.length ?? 0) - 1; i >= 0; i--) {
    if (history[i].role === "assistant") break;
    if (history[i].role === "user" && BOT_OR_HUMAN_QUESTION.test((history[i].content || "").split(/\n\n?\[SYSTEM:/)[0])) return true;
  }
  return false;
}
const TEAM_LINE: Record<"en" | "es" | "pt", string> = {
  en: "You're texting with the OzziFloors team!",
  es: "Estás hablando con el equipo de OzziFloors!",
  pt: "Você está falando com a equipe da OzziFloors!",
};
export function stripHumanClaim(text: string, lang: "en" | "es" | "pt" = "en"): string {
  return withTagsProtected(text ?? "", (prose) => {
    if (!HUMAN_CLAIM.test(prose)) return prose;
    // Clause by clause: "…the OzziFloors team, so a real person is behind this."
    // keeps the first clause; a sentence that is only the claim goes whole.
    const kept = splitSentences(prose).map((s) => {
      if (!HUMAN_CLAIM.test(s)) return s;
      const end = (s.match(/[.!?]+\s*$/) ?? [""])[0];
      const clauses = s.slice(0, s.length - end.length).split(/,\s*/).filter((c) => !HUMAN_CLAIM.test(c));
      const body = clauses.join(", ").trim();
      if (body.replace(/[^\p{L}]/gu, "").length < 12) return "";
      return body.charAt(0).toUpperCase() + body.slice(1) + (end || ". ");
    });
    const out = kept.join("").replace(/[ \t]{2,}/g, " ").trim();
    console.log("[AI] human-claim backstop: removed a 'real person' claim from the reply");
    // Nothing but the claim was there: the owner's own line answers the question.
    if (out.replace(/\[[^\]]*\]/g, "").trim().length < 15) return (TEAM_LINE[lang] + " " + out).trim();
    return out;
  });
}
...
// and a phone (typed, or the WhatsApp number). The name is never checked:
// it is not a booking requirement (owner rule 2026-09-16), the retry note tells
// the model to write the tag with "name":"" when the client never said it.
...
export function bookingItemsGiven(history: Array<{ role: string; content: string }>, phoneKnown: boolean): Set<AskItem> {
  ...
  // Owner rule 2026-09-16: the name is NOT a booking requirement and is never
  // asked, so it always counts as given and any name ask is dropped from the
  // sentence (clientTypedName stays available for diagnostics / evals).
  given.add("name");
  return given;
}
// The items an ask can legitimately still want (the name never, the city only
// when the model itself named it: it is never inferred as missing).
const REQUIRED_ITEMS: AskItem[] = ["address", "zip", "phone"];
...
export function rewriteBookingDataAsk(
  ...
  const out = withTagsProtected(text, (prose) => {
    ...
    const rebuilt = sentences.map((s) => {
      ...
      // A name-only ask ("What name should I put the visit under?") while the
      // address / zip / phone are still missing becomes the ask for THOSE; when
      // nothing is missing it is left alone (the forced-[BOOK] retry's job).
...
export const BOOK_NOW_NOTE =
  "[SYSTEM: BOOKING DATA IS COMPLETE, WRITE THE [BOOK:...] TAG NOW. The client already confirmed a specific day and time, and typed the property address with its ZIP code (the phone is known too). Your previous draft either said the visit was scheduled or asked again for data the client already gave, WITHOUT the [BOOK:...] tag, so NO visit exists in the system. Output now: 5 words or fewer, then the [BOOK:{...}] tag with the exact phone, street address and ZIP the client typed (the name only if the client stated it, otherwise \"name\":\"\"), and the date and time the client confirmed (copy the [YYYY-MM-DD] of that weekday from the schedule). The flooring type is NOT required to book, never ask it before the tag. The client's name is NOT required either (owner rule 2026-09-16): never ask for it, write the tag with \"name\":\"\" when the client never said it and the system fills it in from the profile. Never say the visit is booked, scheduled, agendada or reservada without the tag in the same message.]";
