...
import { gatherWebSources, type SourceItem } from "./sources"
...
export async function runDailyReport(): Promise<void> {
  ...
  // ---- Step 2b: Web/social sources (Reddit grátis + Google/LinkedIn via Firecrawl) ----
  logSection("STEP 2b/7: Web & Social Monitoring (Reddit, Google, LinkedIn)")
  let sourceItems: SourceItem[] = []
  try {
    sourceItems = await gatherWebSources()
    log(`Web sources complete: ${sourceItems.length} item(s) gathered.`)
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
    log(`Web sources step failed (non-fatal): ${msg}`)
    log("Continuing without web/social data...")
  }

  if (videos.length === 0 && repos.length === 0 && sourceItems.length === 0) {
  ...
  try {
    analysis = await analyzeWithClaude(config.anthropicApiKey, videos, repos, sourceItems, memoryContext)
