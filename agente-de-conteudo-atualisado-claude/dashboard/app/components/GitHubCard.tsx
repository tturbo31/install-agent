...
function MarkdownText({ text }: { text: string }) {
  ...
  return (
    <div className="space-y-2">
      {lines.map((line, i) => {
        if (line.startsWith("## "))
          return <h3 key={i} className="text-xs font-bold text-[#8a8a8a] mt-3 mb-1 first:mt-0">{line.replace(/^## /, "")}</h3>;
        if (line.startsWith("# "))
          return <h2 key={i} className="text-xs font-bold text-[#ffffff] mt-3 mb-1 first:mt-0">{line.replace(/^# /, "")}</h2>;
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              <span className="inline-flex items-center gap-1 text-[10px] text-[#6b6b6b]">
                <svg className="w-3 h-3 text-[#8a8a8a]" fill="currentColor" viewBox="0 0 24 24">
            ...
            <a
              ...
              className="text-sm font-semibold text-[#f5f5f5] hover:text-[#ffffff] transition-colors leading-snug line-clamp-1 block"
            >
              {repoName}
            </a>
          </div>
          ...
          {(["overview", "implement"] as Tab[]).map((tab) => (
            <button
              ...
              className={`flex items-center gap-1 px-3 py-1.5 text-[10px] font-semibold transition-all duration-150 border-b-2 -mb-px ${
                activeTab === tab ? "text-[#ffffff] border-[#ffffff]" : "text-[#555] border-transparent hover:text-[#a1a1a1]"
              }`}
            >
                ...
                {showSteps && (
                  <ol className="mt-2 space-y-1.5">
                    {project.howToImplement.map((step, i) => (
                      <li key={i} className="flex items-start gap-2 text-xs text-[#a1a1a1]">
                        <span className="shrink-0 w-4 h-4 rounded-full bg-[#ffffff]/10 text-[#ffffff] text-[10px] font-bold flex items-center justify-center mt-0.5">{i + 1}</span>
            ...
            {project.implementationForOurSystem ? (
              <div className="space-y-1">
                <div className="flex items-center justify-between gap-2 mb-3">
                  ...
                  <SpecButtons
                    size="sm"
                    getText={() => `# Implementar: ${repoName}\n${project.url}\n\n${project.implementationForOurSystem}`}
                    filename={`implementar-${repoName.replace(/[^a-z0-9]+/gi, "-")}`}
                  />
                </div>
